/**
 * PayGuard — Payment Status Guard
 * Lightweight client-side SDK (~2KB)
 * 
 * Usage:
 * <script src="https://your-domain.com/sdk/payment-guard.js" 
...
(function () {
    ...
    if (!scriptTag) {
        console.warn("[PayGuard] No script tag found with data-api-key attribute.");
        return;
    }
    ...
    if (!apiKey) {
        console.warn("[PayGuard] data-api-key is required.");
        return;
    }
    ...
    function checkPaymentStatus() {
        ...
        fetch(url)
            ...
            .catch(function (err) {
                ...
                console.warn("[PayGuard] Could not verify payment status:", err.message);
            });
    }
    ...
    function showBlockPage(projectName, clientName) {
        ...
        card.style.cssText =
            ...
            "box-shadow:0 8px 32px rgba(0,0,0,0.5);animation:payguard-in 0.5s ease;";

        card.innerHTML =
            '<style>@keyframes payguard-in{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:translateY(0)}}' +
            "@keyframes payguard-pulse{0%,100%{opacity:1}50%{opacity:0.5}}</style>" +
            // Warning Icon
            ...
            'animation:payguard-pulse 2s infinite;"></span>' +
